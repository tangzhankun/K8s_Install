...
/root/spark-on-k8s/dist/bin/spark-submit \
  ...
  --conf spark.kubernetes.driver.docker.image=172.16.3.78:4000/spark-driver:b22 \
  --conf spark.kubernetes.executor.docker.image=172.16.3.78:4000/spark-executor:b22 \
  --conf spark.kubernetes.initcontainer.docker.image=172.16.3.78:4000/spark-init:b22 \
  --conf spark.kubernetes.driver.labels="user=calico1,purpose=hdfs" \
  --conf spark.kubernetes.executor.labels="user=calico1" \
  hdfs://172.16.3.68:9000/user/root/spark-examples_2.11-2.1.0-k8s-0.3.1-SNAPSHOT.jar 10 20000 4000
